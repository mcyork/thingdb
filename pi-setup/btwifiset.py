...
class WpaSupplicant:
    ...
    def scan(self):
        ...
        out = subprocess.run("wpa_cli -i wlan0 scan", 
                            shell=True,capture_output=True,encoding='utf-8',text=True).stdout
        mLOG.log(f'wpa_cli scan output: {out}')
        time.sleep(1)
        out = subprocess.run("wpa_cli -i wlan0 scan_results", 
                            shell=True,capture_output=True,encoding='utf-8',text=True).stdout
        mLOG.log(f'wpa_cli scan_results output: {out}')
    ...
    def get_psk(self,ssid,pw):
        ...
        elif len(pw)>=8 and len(pw)<=63:
            # Don't use wpa_passphrase - just return the plain password
            # wpa_cli will handle PSK generation internally when we pass it in quotes
            psk = f'psk={pw}'
        else:
            mLOG.log(f'Password length {len(pw)} is invalid (must be 8-63 characters)')
        mLOG.log(f'psk from get_psk: password={pw}, returning: {psk}')
        return psk
        ...
        try:
            ...
            if ssid_num != '-1':
                if psk == "psk=NONE":
                    ...
                    # Use plain password in quotes - wpa_cli will generate PSK internally
...
class WifiManager:
    ...
    def network_manager_test(self):
        ...
        return network_manager_is_running  # Use NetworkManager if available
